...
class Github {
  ...
  setConfigRepo = (options: any) => {
    ...
    cmd += ` && git config url."https://${this.account.token}@github.com/${this.userName}".insteadOf "https://github.com/${this.userName}"`;
    // cmd += ` && git remote add origin https://github.com/${this.userName}/${options.name}.git`;
    cmd += ` && git remote set-url origin https://${this.userName}@github.com/${this.userName}/${options.name}.git`;
    console.log(cmd);
    execSync(cmd);
  };
